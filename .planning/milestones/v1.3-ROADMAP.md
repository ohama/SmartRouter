# Milestone Archive: v1.3 Quality-Aware ML Routing

**Status:** ✅ SHIPPED 2026-05-11
**Phases:** 1-16 (Phase 17 deferred to v2.0)
**Total Plans:** 62
**Releases:** v1.0.0, v1.1.0, v1.1.1, v1.2.0, v1.3.0

This is the archived roadmap for milestone v1.3. For the current project status, see `.planning/ROADMAP.md` (created fresh for v2.0). For milestone summary, see `.planning/MILESTONES.md`.
...
- [x] **Phase 16: 122B-as-Judge for Borderline Cases** ✓ — distillation Tier 3-A lazy verification 구현. 4 plans / 3 waves: 16-01 `Adapters/BorderlineClassifier.fs` (BCL-only; `BorderlineKind = UncertainEntropy | UncertainLength` 별도 DU — Verdict 미확장; classifyBorderline entropy band → length band 캐스케이드; keyword/finish_reason **의도적 제외** = binary 신호 no uncertainty band); 16-02 `Adapters/JudgeClient.fs` (`IJudgeClient` + `IJudgeStats` 별도 인터페이스 + `JudgeVerdict` 4-case DU; named "judge" HttpClient Phase 7 TeacherLabeler 패턴 미러; `ConcurrentDictionary<(prompt_hash, response_hash), CacheEntry>` LRU + O(n) `Seq.minBy` eviction + monotonic int64 access counter; ROUTE_NO-wins-on-collision 안전 편향 parser; 1-token max_tokens + JsonFSharpConverter; `prompts/judge-prompt.md` 운영자 튜닝 템플릿); 16-03 wiring (`ChatCompletions.fs` non-streaming: judge `GetService` null-safe resolve, `computePromptHash` hoist 공유, Good arm + Qwen35B + classifyBorderline Some _ 일 때만 judge 호출, RouteNo+retry-fails → judgeTriggeredFallback=`false` **B3 fix** = trace 가 judge 발화는 기록하되 substitution 미발생 시 fallback_kind=null, RouteYes/Skipped/Failed fail-open; `TraceRecord` 13 → 16 fields additive schema_version=1 unchanged; `Stats.fs` 3 새 flat snake_case 카운터; `CompositionRoot.fs` clean two-branch if/else **B4 fix** — last-registration-wins 패턴 안 씀; `appsettings.json:Routing.Judge` 5-key OPT-IN `Enabled=false`); 16-04 tests + docs (`JudgeIntegrationTests.fs` 11 cases; README §5.5.5/§7/§8/§9.3; CHANGELOG `[Unreleased] ### Added` OPT-IN 톤; REQUIREMENTS.md JDG-01 본문 **B1 fix** — keyword 제외 / BorderlineKind 별도 DU / Verdict 미확장 근거 명시). **Notable deviation from ROADMAP literal text** (B1): JDG-01 ROADMAP wording 은 keyword borderline 차원 포함 + 3-way Verdict 확장 제안; 합의는 keyword 제외 (binary; no natural uncertainty band) + BorderlineKind separate DU (Verdict 확장 시 6 pattern-match 영향) → REQUIREMENTS.md 정렬 업데이트로 해결. Streaming branch INTENTIONALLY SKIPPED preserved. Judge feature **OPT-IN by default** (vs Phase 15 silent-enable) — judge 가 122B network call 비용 추가하므로 operator 평가 후 활성화. **113 passed + 16 ignored + 0 failed** (was 102+16). gsd-verifier: 14/14 must-haves passed. ARCH-01 preserved.
- [~] **Phase 17: QualityClassifier (distillation endgame) — DEFERRED 2026-05-11** — Originally planned as Tier 4 (ML.NET binary classifier on `bge-m3 embed(prompt) ⊕ bge-m3 embed(response)` 2048-dim → good/bad; closed-loop self-improvement from trace + decision + judge data). **Operator pivot 2026-05-11**: ML routing 자체를 dormant 시키고 selfrouting 도입 (`.planning/docs/35b-selfrouting.md`); Phase 17 ML classifier 도 자연스럽게 보류. Plans 안 만들어짐; QCLS-01..07 requirements 는 v2.0 closeout 시 deferred 마킹. ML 코드 (Phase 6-9 + 14-16) 는 retained but dormant — 미래 부활 시 옵션 C (`Routing.Mode = "ml" \| "selfrouting"`) 패턴 가능. `archive/v1.3-ml-routing` branch + `v1.3.0` tag 가 historical snapshot.
...
| 16. 122B-as-Judge for Borderline Cases | 4/4 | ✓ Complete | 2026-05-11 |
| 17. QualityClassifier (distillation endgame) | 0/5 | ⏸ Deferred (operator pivot to selfrouting) | 2026-05-11 |
