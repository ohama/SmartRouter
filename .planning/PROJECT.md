...
| **Embedding model: bge-m3 from Phase 6** (skipping bge-small MVP entirely) | Operator's traffic mixes Korean+English. bge-small (-en) cannot tokenize Hangul — Korean prompts route at random per `~/projs/smart-router-distillation/docs/embedding-classifier-decision-deep-dive.md` §1.7.1. The doc's measure-then-migrate philosophy was designed for English-mostly setups; operator confirmed Korean ≥ 20% in advance, so the measurement step is unnecessary overhead. Cost trade-off: ~5x embedding latency on CPU (50-80ms vs 10-15ms), 2.3GB model file vs 130MB. Mitigation: EMBED-03 latency budget with int8/CoreML EP fallback. | — Pending |
