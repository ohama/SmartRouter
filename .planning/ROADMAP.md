...
- [ ] **Phase 13: Service Logging** — Phase 13 prep complete (CONTEXT + 6 plans drafted at `.planning/phases/13-service-logging/`); awaits execution. Adds dual sink (stderr kept + rolling File at logs/operational/smart-router-{Date}.log, 50MB cap, 30-day retention), `[{correlation_id}]` rendering bracket, `appsettings.json:Serilog` Override table activated, `--log-level=enum` CLI flag (replaces --trace), full ILogger<T> migration across 19 emitting source files, hot-path log demotion in ChatCompletions, HealthService transition-only logging, 4 endpoint DEBUG hits, startup + shutdown banners, LogRetentionService BackgroundService prunes decision JSONL > 90 days + teacher-cap > 7 days, 13+ new tests in LogRotationTests.fs, README "Operational Logging" expanded section.
...
| 13. Service Logging | 0/6 | ⏳ Planned | — |
