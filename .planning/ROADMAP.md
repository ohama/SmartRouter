...
Plans:
- [ ] 06-01: ONNX export `BAAI/bge-m3` AND apply int8 dynamic quantization (`optimum-cli export onnx` + `onnxruntime.quantization.quantize_dynamic` OR `optimum-cli` with `--quantize` if available; documented script under `scripts/export-bge-m3-int8.sh`); add `Microsoft.ML.OnnxRuntime` + tokenizer NuGet (research-time decision: `Microsoft.ML.Tokenizers` with SentencePiece OR `BERTTokenizers` adapted for XLM-R) + `Microsoft.ML` + `Microsoft.Extensions.ML` pins; verify versions live; design `IEmbedder` (1024-dim) and `IClassifier` ports
- [ ] 06-02: Implement `BgeM3Embedder` (loads int8 ONNX, mean pooling + L2 normalize; warm-up call on cold start to amortize JIT) and `MlNetClassifier` adapters; first-run dummy 1024-dim model generator; replace placeholder `applyML`; **flip `appsettings.json` `Routing.Algorithm` default from `"heuristic"` to `"ml"`** (heuristic remains in code as dormant fallback per the 2026-05-08 soft-pause decision; CompositionRoot dispatch preserves all 4 input cases)
- [ ] 06-03: Tests — embedding determinism on 3 prompts (en/ko/mixed), classifier load+predict, a/b decision divergence including Korean prompt, dummy bootstrap, model_version tagging in DecisionLog, **p95 latency benchmark under 50ms (EMBED-03)** with explicit warm-vs-cold separation
