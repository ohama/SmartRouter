...
- [ ] **Phase 9: Canary Deployment** — `Microsoft.FeatureManagement.AspNetCore` + `ContextualTargetingFilter` for sticky 10/90 split keyed on `correlation_id` (NOT `PercentageFilter` — research found it's non-sticky); `RoutingDecision.ModelVersion` cohort label propagated from ML.fs (single isCanary boolean gates classifier-selection AND model_version); rolling-60s fallback-rate watchdog with `AutoRollbackEnabled=false` default until Phase 10's `fallback_used` signal is real; `/canary` admin endpoint (status / promote / rollback / enable) coordinates with Phase 8 RetrainingService via shared `IRetrainLock`
...
Plans:
- [ ] 09-01-PLAN.md — Foundation: add `RouterRequest.CorrelationId` + `RoutingDecision.ModelVersion` fields (Core domain change touching all 19 enumerated construction sites); new `SmartRouter.Core/CanaryPorts.fs` BCL-only `ICanaryGate` port; `IModelVersionProvider` extended with `CanaryVersion` getter + `UpdateCanary` setter; `ML.fs makeApplyML` factory gains 4 new params (canaryClassifier, canaryGate, baselineVersion, canaryVersion) — single `isCanary` boolean gates classifier selection AND `decision.ModelVersion` (Pitfall 8); `ChatCompletions` threads `correlationId` into `mapWireToRequest` and cascades `decision.ModelVersion` → `versionProvider.CurrentVersion`; appsettings gains `Canary` + `feature_management` sections; `Microsoft.FeatureManagement.AspNetCore` 4.5.0 NuGet pin; existing 73+10 tests preserved.
- [ ] 09-02-PLAN.md — Implementation: 7 new Cli adapters (CanaryTargetingAccessor / CanaryState / CanaryGate / CanaryMetrics / CanaryWatchdog / RetrainLock / CanaryService) + 1 endpoint file (Endpoints/Canary.fs); `MlNetClassifier` ctor parameterized on modelName; PredictionEnginePool gains second `.FromFile("router-canary", ...)` chain; CompositionRoot wires `AddScopedFeatureManagement().WithTargeting<CanaryTargetingContextAccessor>()` + AddHttpContextAccessor + 9 new singletons + AddKeyedSingleton<IClassifier> for baseline/canary; RetrainingService refactored to acquire shared `IRetrainLock` (Phase 8 cancellation pattern preserved verbatim); ChatCompletions records `metrics.Record(isCanary, isFallback)` post-response in 6 Ok-decision arms; existing 73+10 tests still pass.
- [ ] 09-03-PLAN.md — CanaryTests.fs (5 unit + 6 integration tests; testSequenced): CANARY-01 statistical split (1000 distinct UUIDs at 10% → count in [80,120]) + sticky bucket (same UUID → identical) + 3 short-circuit guards (empty correlation_id, missing canary file, ICanaryState percentage=0); CANARY-02 cohort tagging in JSONL DecisionLog at 50% rollout; CANARY-03 manual rollback / enable / promote-success / promote-no-canary endpoint integration; CANARY-03 auto-rollback fires when canary fb_rate exceeds baseline + threshold (CapturingSink ILogEventSink for `AUTO-ROLLBACK fired` log assertion) AND auto-rollback disabled by default (Lock 1); target test count ≥80 passed + ≥10 ignored + 0 failed.
