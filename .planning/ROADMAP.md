...
- [x] **Phase 15: Quality Signal Enrichment** ✓ — Phase 14 `isBadResponse` heuristic 을 5 dimensions 로 풍부화. 3 plans / 3 waves: 15-01 foundation (`BadReason`/`Verdict` DUs, 5 BCL-only helpers — `extractFinishReason`/`koreanRatio`/`effectiveLength`/`charEntropy`/`matchKeyword`, `QualityFallbackOptions` extended with `BadFinishReasons`/`EntropyThreshold` + silent-enable defaults); 15-02 wiring (`analyzeResponse` cheap-first cascade `finish_reason → length → entropy → keyword`, `isBadResponse` backward-compat wrapper, `IQualityCheckStats` interface on `QueueDispatcher` with 4 `Interlocked` counters, `StatsWire` 4 flat snake_case fields `quality_check_hits_*`, `TraceRecord` 13th `bad_reason: string option` field with `tag=value` serialization, `ChatCompletions` non-streaming branch wired with DI in both composition paths via NoOp fallback for `configureWithoutMl`); 15-03 tests + docs (`QualitySignalEnrichmentTests.fs` 14 cases covering 5 dimensions + cascade + integration test asserting `bad_reason="finish_reason=length"` in trace + counter increments, README §5.5/§7/§9.3/§8 updated, CHANGELOG `[Unreleased] ### Changed`/`### Added` with silent-enable migration tone + restore-Phase-14 instructions). **Notable deviation from ROADMAP literal text**: QSE-03 wording proposed default `BadKeywords` expansion to 7 refusal patterns; CONTEXT.md explicitly forbids this and requires operator opt-in via README §7 guidance — default stays `["TODO", "I think"]`. Streaming branch INTENTIONALLY SKIPPED preserved (chunks already shipped). schema_version=1 unchanged (additive trace field). **102 passed + 16 ignored + 0 failed** (was 88+16). gsd-verifier: 13/13 must-haves passed. ARCH-01 preserved (zero Core changes).
...
Plans:
- [ ] 15-01-CONFIG-AND-DOMAIN-PLAN.md — `QualityFallbackOptions` 확장 (`BadFinishReasons: string array` + `EntropyThreshold: float`); BadReason/Verdict DUs + 5 pure helpers (koreanRatio, effectiveLength, charEntropy, matchKeyword, extractFinishReason) in `Adapters/QualityCheck.fs`; `normalizeQualityFallback` + `appsettings.json:Routing.QualityFallback` 신규 default 추가. Default `BadKeywords` 는 Phase 14 `["TODO","I think"]` 그대로 유지 (CONTEXT.md §refusal-default policy — refusal patterns operator opt-in via README; ROADMAP SC#3 wording deferred to README guidance).
- [ ] 15-02-ANALYZERESPONSE-AND-WIRING-PLAN.md — `analyzeResponse` cheap-first cascade (finish_reason → effective length → entropy → keyword) returning `Verdict`; `isBadResponse` 는 backward-compat wrapper 로 보존 (호출자 한 곳 영향 적음 + Phase 14 unit tests 보존). `IQualityCheckStats` 별도 인터페이스 + `QueueDispatcher` 에 4 int64 counters 구현; `StatsSnapshot.QualityCheckHits` + `StatsWire` 4 snake_case 필드 추가; `TraceRecord.bad_reason: string option` (13th field, schema_version=1 유지). `ChatCompletions.fs` non-streaming branch 가 `extractFinishReason` → `analyzeResponse` → `Verdict` match → counter 증가 + bad_reason 직렬화. Streaming branch INTENTIONALLY SKIPPED 그대로. `IQualityCheckStats` 두 composition path (configureRequestPipeline + configureWithoutMl) 모두 등록.
- [ ] 15-03-TESTS-AND-DOCS-PLAN.md — `QualitySignalEnrichmentTests.fs` (~8-9 testCases: QSE-01..06 + cheap-first cascade order + Phase 14 wrapper 회귀 + 1 fake-Kestrel 통합 테스트로 finish_reason="length" 흐름 + bad_reason trace + /stats counter 증가 검증). README §5.5/§7/§9.3/`/stats` 섹션 업데이트; CHANGELOG `[Unreleased] ### Changed + ### Added` entries per CONTEXT.md migration tone; `.planning/docs/quality-check-improvement-options.md` Tier 1+2 implemented status.
...
| 15. Quality Signal Enrichment | 3/3 | ✓ Complete | 2026-05-10 |
