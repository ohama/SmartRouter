...
- [ ] **Phase 17: Hard Rules Layer + Routing.Mode Switch** — Stage 0 keyword pre-routing + dormant-ML config gate; foundation for the selfrouting cascade
- [ ] **Phase 18: Session Store + Sticky Escalation** — `RouterRequest.SessionId` field, `ISessionStore` adapter with TTL eviction, sticky-122B continuation logic
- [ ] **Phase 19: 35B Self-Routing** — Named `selfrouter` HttpClient + 1-token SAFE/UNSAFE classify + prompt-hash LRU cache + cascade integration (non-streaming only)
...
2. **Explicit override and explicit task bypass Hard Rules**: Sending `{"model": "35b", "messages": [{"content": "LLVM optimization"}]}` routes to 35B (model override wins); sending `{"task": "retrieval", "messages": [{"content": "MLIR debugging"}]}` routes to 35B (task table wins). Verified by unit tests in `HardRulesTests.fs` that exercise the cascade ordering.
...
Plans:
- [ ] 17-01: Core Hard Rules + DU extensions — `SmartRouter.Core/HardRules.fs` BCL-only pure function (`applyHardRules: RouterRequest -> RoutingDecision option`); `RoutingReason.HardRule` 7th DU case; `DecisionLogger.formatReason` exhaustive 7-arm match → `"hard_rule"`; default keyword list hardcoded; unit tests for all 6 keywords + case-insensitivity + no-match passthrough
- [ ] 17-02: Routing.Mode config + cascade wiring — `appsettings.json:Routing.Mode` key with validation (fail-fast on unrecognized value); `CompositionRoot.configureRequestPipeline` two-branch on `Routing.Mode` (selfrouting → register Hard Rules + stub SelfRouter placeholder + reserved SessionStore; ml → existing v1.x path); ChatCompletions.fs Stage 0 Hard Rules call in BOTH streaming and non-streaming branches; explicit override + explicit task cascade ordering verified by integration test
- [ ] 17-03: README + CHANGELOG + tests — README §5.5 routing pipeline diagram updated, §7 `Routing.Mode` config reference added; CHANGELOG `[Unreleased] ### Added` (Hard Rules layer) + `### Changed` (v2.0 mode default); integration test fixture mlx_lm fakes + DecisionLog grep for `routing_reason=hard_rule`; baseline test count update
...
|-------|-----------|----------------|--------|-----------|
| 17. Hard Rules + Routing.Mode | v2.0 | 0/3 | Not started | - |
