...
- [ ] **Phase 10: Health + Fallback + graph_indexing No-Fallback** — (was old Phase 4) Health probing (BackgroundService + ConcurrentDictionary state), retry policy on non-streaming HttpClients (4 named clients: stream vs non-stream), fallback policy in ChatCompletions pre-flight (35B reroute for non-graph_indexing; 503 for graph_indexing) with `fallback_used=true` activating Phase 7-8 ML feedback loop
...
Plans:
- [ ] 10-01-PLAN.md — Foundation: `RoutingReason.FallbackTo35B` DU case (+ exhaustive `formatReason` cascade); extend `IHealthProbe` port with sync `IsReachable` + `LastProbedAt` (BCL-only, ARCH-01 safe); add `Routing.Health` config (`PollingIntervalSeconds=10`, `ConsecutiveFailureThreshold=1`); flip `Canary.AutoRollbackEnabled` to `true` (Phase 9 was held back until Phase 10's signal becomes real); preserve 78+17 baseline.
- [ ] 10-02-PLAN.md — Implementation: `HealthService.fs` BackgroundService (PeriodicTimer probes both upstreams via `/v1/models`; `ConcurrentDictionary` state + per-target consecutive-failure counter; triple-reg DI + 5 named HttpClients — 2 with retry, 2 stream-only-no-retry, 1 health-probe); `Endpoints/Health.fs` GET /health; QueueDispatcher takes `IHealthProbe` (defensive belt-and-suspenders); ChatCompletions pre-flight check BEFORE SSE headers (graph_indexing-must-fail → 503 + structured error body; non-graph_indexing 122B-down → reroute to 35B via local `decision` rebind so existing log calls pick up `IsFallback=true` automatically); QwenUpstreamClient `resolveProbe` extends to 4-way (target × stream) client-name map; preserve baseline test count.
- [ ] 10-03-PLAN.md — Tests: `HealthFallbackTests.fs` with 5 integration tests (HLTH-04 reasoning fallback / HLTH-05 graph_indexing-503 / HLTH-06 transient retry / HLTH-07 streaming no-retry / HLTH-08 /health JSON shape + flip-on-stop) using `testSequenced` + temp-dir + fake-Kestrel pattern from StreamingTests.fs; register in RouterTests rootTests; target ≥83+17 (or ≥90+10 with embeddings).
